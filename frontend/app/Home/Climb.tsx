...
export default function ClimbScreen() {
  ...
  useEffect(() => {
    ...
    setHolds([
      { id: '1', type: 'Crimp', nextHold: undefined, position: { top: 20, left: 30 }, size: { width: 20, height: 20 } },
      { id: '2', type: 'Sloper', nextHold: '1' , position: { top: 40, left: 60 }, size: { width: 20, height: 20 } },
      { id: '3', type: 'Foothold', nextHold: '2', position: { top: 60, left: 20 }, size: { width: 20, height: 20 } },
      { id: '4', type: 'Jug', nextHold: '3', position: { top: 80, left: 70 }, size: { width: 20, height: 20 } },
    ]);
    setLoading(false);
  }, []);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.topBar}>      
        <TouchableOpacity style={[styles.topButton, { backgroundColor: colors.primary }]} onPress={() => navigation.goBack()}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.mainSection}>
          <View style={[styles.gradesContainer, { backgroundColor: colors.primary }]}>
                  ...
                  <TouchableOpacity
                    ...
                    style={[
                      styles.hold,
                      {
                        ...
                        backgroundColor: selectedHold === hold.id ? '#ff0080' : '#333',
                        width: hold.size.width,
                        height: hold.size.height,
                      }
                    ]}
                    onPress={() => setSelectedHold(selectedHold === hold.id ? null : hold.id)}
                  />
                ))}
              </View>
            </View>
                ...
                {selectedHold && (
                  <>
                    {(() => {
                      ...
                      const nextHold = hold?.nextHold ? holds.find(h => h.id === hold.nextHold) : null;
                      return hold ? (
                        <>
                          <Text style={styles.holdType}>Selected Hold: {hold.type}</Text>
                          {nextHold && (
                            <TouchableOpacity 
